...
type RiverProps = {
	...
	strokeWidth?: number;
	opacity?: number;
	/** 0-1+ gold glow intensity, e.g. once a river/point is "identified". */
	glow?: number;
	style?: CSSProperties;
};
...
// A river rendered as a code-drawn SVG path (dark ink line, gold glow when
// highlighted) instead of a raster image. Normalized to pathLength=100 so
// stroke-dashoffset reveals it frame-accurately regardless of the curve's
// actual geometry. Once mostly drawn, a second dashed pass continuously
// marches along the path so it never reads as merely faded in.
	...
	return (
		...
		>
			<path
				...
				style={glow > 0 ? {filter: `drop-shadow(0 0 ${glowRadius}px rgba(212,175,55,${Math.min(glow, 1) * 0.9}))`} : undefined}
			/>
			{flowFadeIn > 0 && (
				<path
					d={d}
