...
export const breathe = (frame: number, fps: number, amplitude = 0.02, hz = 0.22) =>
	1 + amplitude * Math.sin((frame / fps) * 2 * Math.PI * hz);

/** Slow vertical drift in pixels, e.g. for idle holds. */
export const driftY = (frame: number, fps: number, amplitude = 5, hz = 0.18) =>
	amplitude * Math.sin((frame / fps) * 2 * Math.PI * hz);

/** Faster pulse, e.g. for a pin or glow that should read as "alive". */
